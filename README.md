...
一个“犀利但鼓励”的 AI 产品经理评审 Agent。MVP 闭环是：输入产品想法，生成结构化评审报告，保存历史，查看详情，复制开发 Prompt，导出 Markdown，并支持把同一个产品想法迭代成 V1/V2/V3。

第一版不实现完整登录、支付、分享链接、复杂管理后台、多 Agent、RAG 和自动部署。

当前 P7 版本新增：

- 想法版本管理：基于旧报告继续优化，生成同组 V2/V3。
- 迭代版本对比：查看同一想法组的分数、决策、MVP 和验证计划变化。
- 开发任务包：基于报告拼接可复制、可下载的 Codex/Cursor 开发任务包。
- 输入提示引导：帮助用户按目标用户、真实痛点、替代方案、核心功能和验证目标组织输入。
